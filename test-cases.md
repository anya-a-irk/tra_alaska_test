...
**Ожидаемый результат**
```status code 200; Error. Pls enter existing bear id```
...
1. Отправить GET запрос (```curl -X GET -i 'http://172.17.0.1:8091/bear/:id'```)
...
```curl -X PUT -i 'http://172.17.0.1:8091/bear/:id' --data '{"bear_type":"BLACK","bear_name":"mikhail","bear_age":17.5}' ```)
3. Отправить GET запрос (```curl -X GET -i 'http://172.17.0.1:8091/bear/:id'```)
...
**Ожидаемый результат**
```status code 200; Error, pls enter correct bear id ```
